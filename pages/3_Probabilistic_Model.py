...
def plot_prob_forecast(
    ...
    plt.title(f"Quantile-XGBoost Forecasting {forecast_step}-steps ({forecast_step} hours) ahead for {selected_household_id}")
...
def probabilistic_page():
    ...
    st.header("Probabilistic Model (Quantile-XGBoost)")
    ...
    with col4:
        st.metric(label='R2', value=round(r2, 4))
    ...
    st.header("Forecast with Quantile-XGBoost")
